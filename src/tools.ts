...
import { executeAstGrep, formatMatchesAsText, getSupportedLanguages } from "./utils";
...
export function createFindTool(directory: string) {
  return tool({
    ...
    async execute(args) {
      ...
      const { matches, stdout } = await executeAstGrep("run", cmdArgs, {
        directory,
      });
...
export function createFindByRuleTool(directory: string) {
  return tool({
    ...
    async execute(args) {
      ...
      const { matches, stdout } = await executeAstGrep("scan", cmdArgs, {
        directory,
      });
